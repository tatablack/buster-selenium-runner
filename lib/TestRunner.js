...
//            || (!fs.existsSync(path.join(configPath, nconf.get('busterRunner').spec))) ) {
    ...
    function runTests() {
        ...
        if (!fs.existsSync(path.join(configPath, 'test-results'))) fs.mkdirSync(path.join(configPath, 'test-results'));

        nconf.get('busterRunner').specs.forEach(function(spec) {
            runTasks[runTasks.length] = (function(spec, index) {
                return function() {
                    var busterRunner = new BusterRunner(path.join(configPath, 'test-results', 'junit-output-' + index + '.xml'));
