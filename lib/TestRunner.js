...
//            || (!fs.existsSync(path.join(configPath, nconf.get('busterRunner').spec))) ) {
    ...
    function runTests() {
        ...
        nconf.get('busterRunner').specs.forEach(function(spec) {
            var tempBusterRunner = new BusterRunner(path.join(configPath, 'junit-output-' + testRunDeferred.length + '.xml'));

            testRunDeferred[testRunDeferred.length] = when(
                nodefn.call(
                    tempBusterRunner.execute,
                    path.join(configPath, spec),
                    busterServer.getUrl(nconf.get('busterServer').interface)
                ),
                function() {
                    console.log('[TestRunner] ..tests for %s completed.', spec);
                    return true;
                },
        ...
        return when.all(testRunDeferred);
    }
]).then(
    undefined,
    function(err) {
