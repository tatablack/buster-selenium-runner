...
function BusterServer(networkInterface) {
    process.on("uncaughtException", function (err) {
        console.error('[BusterServer] Unhandled exception: %s', err);
    });
    ...
    function getIpAddressFor(iface) {
        ...
        if (!interfaces[iface]) throw new Error('the requested network interface (' + iface + ') is not available. Check your config.json file.');
        ...
        start: function() {
            ...
            when(
                ...
                function(availablePort) {
                    ...
                    console.log('\n[BusterServer] Starting buster server on port %d', usedPort);
