...
function SeleniumGridConsole(gridConfig) {
    ...
    return {
        ...
        checkAvailability: function() {
            console.log('\n[Grid] Checking node availability on selected Selenium Grid (' +
            ...
            grid.available({
                ...
                if (err) {
                    ...
                    var filteredNodes = _.where(available, config.capabilities);
                    ...
                    if (filteredNodes.length === 0) {
                        deferred.reject(new Error('[Grid] Unable to find nodes with the required capabilities. Exiting.'));
                    } else {
                        console.log('[Grid] Browser %s found on grid server', color.bold(config.capabilities.browserName));
